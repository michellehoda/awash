...
or they just had to make decisions about formatting that don't match with what you need.
Sometimes, you get data from a place that wasn't even set up to be data for anyone else,
...
If a case has a missing value, and that attribute is used in a plot or calculation, 
...
Some of the Census data portals, for example, use a huge number to indicate "missing."
Two things to note:
...
the data by `Sex`^[This is "sex at birth," which is what the Census used at the time.]
and computed an aggregate value: the mean income.
He concludes that women make more than men 
because their mean income is nearly 1.9 million dollars while the men make only about 
1.6 million.^[This actually happens in class with smart, well-prepared high-school seniors.]
...
There are two basic strategies: do it in CODAP or do it elsewhere. Let's do CODAP first.
...
An alternative strategy---which is sometimes better than using CODAP for things like this---
is to export the data from the table, edit the **csv** file in a text editor, and then 
...
In some Census data, this value is the "percent of the poverty level" for that person,
...
do a find for `9999999`, put nothing in the **Replace** box, and choose **Replace all**.
...
If I just copy that part of the web page and paste it into a text editor, I see this:
