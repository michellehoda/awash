...
```{r, echo=FALSE}
htmltools::includeHTML("htmls-and-sources/NYC-buildings.html")
```
...
`r codapLink("https%3A%2F%2Fcfm-shared.concord.org%2FjSf1mvRqYTboreKC6qYU%2Ffile.json","Click this to make a new tab with the wolf tracking data.")`
...
```{r, echo=FALSE}
htmltools::includeHTML("htmls-and-sources/land-use-by-state.html")
```
...
As an example, here is a picture of a case card from a dataset about vehicle use by State.
We have selected Oregon, so you can see its values:

```{r echo = FALSE, out.width = "50%", fig.align='center',fig.show='hold', fig.cap="Case card with Oregon vehicle data"}
knitr::include_graphics(c("art/vehicle-state-data-case-card.png"))
```

If you have a dataset with a column that has the names or IDs of some _common_ 
boundary type---let's say US States, as in our example---and you drag an attribute into the map, _nothing happens_.

(xxx make live example here xxx)
...
1. You need a special function. Under **---Insert Function--**, choose the **Lookup functions** section.

1. Choose **lookupBoundary**. You will see this:

```{r echo = FALSE, out.width = "50%", fig.align='center',fig.show='hold', fig.cap="after inserting `lookupBoundary`."}
knitr::include_graphics(c("art/lookup-boundary.png"))
```

As you can see, there are two arguments: **boundary_set** and **boundary_key**. For **boundary_set**, 
you need a special name, which you may not remember.

5. Select **boundary_set** and open the **---Insert Value---** menu. 
The lower part of that list has special names such as $\pi$---and also the various boundary sets that are currently installed.
For our example, choose **US_state_boundaries**. 
(You can type it in if you like; CODAP will help with typeahead.)

1. Finally, replace **boundary_key** with the name of the attribute that has the "key," 
in this case, the one that has the names of states in it. In our case, this is `State`.

Now, if we drag `VehicleMilesTraveled` into the map, we see this:

```{r echo = FALSE, out.width = "100%", fig.align='center',fig.show='hold', fig.cap="Map colored by vehicle miles traveled"}
knitr::include_graphics(c("art/vehicle-miles-traveled-by-state.png"))
```

> Note: The State attribute does not have to have the full name of the State in it. 
...
this one is called **GA-13-georgia-counties.json**.
It begins like this:
...
we find them in an HTML table and process that (see the [chapter on munging](#preparing-chapter)) 
to get a file called **Georgia votes by county.csv**. 
It begins like this:
...
1. Drag the _data_ file into CODAP.
This creates a second, more straightforward table, with the votes 
and the county names.

At this point you will see something like the document in the live illustration below.
...
4. Click on the **boundary** column header in the **votes** table.
A menu appears.
Choose **Delete Formula (Keeping Values)**. 

1. In the **Tables** tool, delete the **geography** dataset.
Delete, hide, or get rid of the formulas of any other geographical attributes in the **votes** table, as you see fit.

1. If you don't have one already, make a map. You'll see the Georgia counties!

1. Now drag a data attribute (such as **Biden**) into the map. Yay! We get a colored map!

```{r, echo=FALSE}
htmltools::includeHTML("htmls-and-sources/GA-2020-election.html")
```

5. Now you can set yourself a task, such as making a CNN-worthy
...
> Teachers, note: this is not just a mechanical exercise! 
...
1. Go to the attribute's menu and choose **Edit Attribte Properties...**. A dialog appears.
